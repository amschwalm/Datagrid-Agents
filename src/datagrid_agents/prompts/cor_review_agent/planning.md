...
8. ASSEMBLE THE REPORT - Start from the shipped template at the end of the custom instructions and fill it in; do not rebuild the document from memory. Copy the head and the `<style>` block verbatim, then work down the template in its own order: masthead identity -> verdict deck -> headline metrics -> three pillar cards -> Verdict -> Review Summary -> Document Inventory -> Cost Breakdown & Validation -> Findings -> Next Steps -> Procore References. Map, mechanically:
